...
class GenDataIter(DPDataLoader):
    """ Toy data iter to load digits"""

    def __init__(self, data_file, batch_size):
        self.dataset = self.read_file(data_file)
        ...
        self.idx = 0
        self.num_workers = 0
        self.sample_rate = batch_size / self.data_num
        super(GenDataIter, self).__init__(dataset = self.dataset, sample_rate=self.sample_rate, num_workers=2)
    ...
    def next(self):
        batch_size = int(self.sample_rate * self.data_num)
        if self.idx >= self.data_num - batch_size:
            raise StopIteration
        index = self.indices[self.idx:self.idx + batch_size]
        d = [self.dataset[i] for i in index]
        d = torch.LongTensor(np.asarray(d, dtype='int64'))
        data = torch.cat([torch.zeros(batch_size, 1).long(), d], dim=1)
        target = torch.cat([d, torch.zeros(batch_size, 1).long()], dim=1)
        self.idx += batch_size
        return data, target
...
class NewGenIter(DPDataLoader):
    """ Toy data iter to load digits"""

    def __init__(self, data_file, batch_size):
        
        self.data_lis = self.read_file(data_file)
        self.dataset = self.data_lis
        ...
        self.num_batches = int(
            math.ceil(float(self.data_num) / batch_size))
        self.idx = 0
        super(NewGenIter, self).__init__(dataset = self.dataset, sample_rate=self.sample_rate, num_workers=2)
...
class DisDataIter(DPDataLoader):
    """ Toy data iter to load digits"""
    ...
    def __init__(self, real_data_file, fake_data_file, batch_size,length):
        ...
        self.indices = range(self.data_num)
        self.sample_rate = self.batch_size / self.data_num
        self.num_batches = int(
            math.ceil(float(self.data_num) / self.batch_size))
        self.idx = 0
        super(DisDataIter, self).__init__(dataset = self.dataset, sample_rate=self.sample_rate, num_workers=2)
