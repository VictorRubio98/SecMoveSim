...
def main(opt):
    ...
    random.seed(SEED)
    np.random.seed(SEED)
    gen_privacy_engine = PrivacyEngine()
    dis_privacy_engine = PrivacyEngine()
    ...
    if opt.pretrain:
        ...
        logger.info(f'Usign device {device}')
        ...
        dis_data_iter = DisDataIter(pretrain_real, pretrain_fake, batch_size=BATCH_SIZE, length=SEQ_LEN)
        dis_criterion = nn.NLLLoss(reduction='sum')
        dis_optimizer = optim.Adam(discriminator.parameters(),lr=0.000001)
        dis_criterion = dis_criterion.to(device)

        discriminator, dis_optimizer, _ = dis_privacy_engine.make_private(
        ...
        generator, gen_optimizer, _ = gen_privacy_engine.make_private(
    ...
    print(f'Generator\'s epsilon: {gen_privacy_engine.get_epsilon()}')
    print(f'Discriminator\'s epsilon: {dis_privacy_engine.get_epsilon()}')
